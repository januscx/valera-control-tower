...
| Arm control | A simulation-only bridge maps the existing `/valera/arm/command` JSON contract to a `JointTrajectory` goal for the six SO-101 joints and leading gripper joint. |
...
- `sim_base_bridge` and the arm-command bridge;
...
| gateway | `/valera/arm/command` | `std_msgs/msg/String` | sim arm bridge | Existing raw JSON event. |
| sim arm bridge | `/valera/arm_controller/follow_joint_trajectory` | `control_msgs/action/FollowJointTrajectory` | `joint_trajectory_controller` | Map only allow-listed named joints; clamp/reject out-of-range values and produce observable failure results. |
...
The arm controller allow-list is `shoulder_pan`, `shoulder_lift`,
`elbow_flex`, `wrist_flex`, `wrist_roll`, and `gripper`.
`left_clamp` remains a URDF mimic joint and has state but no independent command
interface. The bridge uses the URDF limits as a hard validation boundary and
reports rejected, malformed, stale, or unexecutable simulated commands without
...
- Arm bridge tests cover JSON validation, allow-listing, joint limits, mimic
  exclusion, and no hardware imports or device access.
...
- A valid arm command reaches the named simulated joints; no independent
  `left_clamp` command exists.
